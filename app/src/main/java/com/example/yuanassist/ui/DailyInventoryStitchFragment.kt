...
class DailyInventoryStitchFragment : Fragment() {
    ...
    private fun loadExampleImage(view: View) {
        ...
        val resultExampleView = view.findViewById<ImageView>(R.id.iv_daily_inventory_stitch_result)
        try {
            requireContext().assets.open("xingshishili.jpg").use { stream ->
                startExampleView.setImageBitmap(BitmapFactory.decodeStream(stream))
            }
            requireContext().assets.open("xingshijieguo.jpg").use { stream ->
                resultExampleView.setImageBitmap(BitmapFactory.decodeStream(stream))
            }
        } catch (e: Exception) {
    ...
    private fun startInventoryStitchService() {
        ...
        try {
            ...
            Toast.makeText(context, "已准备星石拼图，请点击悬浮窗开始按钮", Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
