...
object NoteDialogs {
    ...
    fun showListDialog(
        ...
        fun refreshList() {
            ...
            if (notedTurns.isEmpty()) {
                listContainer.addView(TextView(themeContext).apply {
                    text = "暂无备注，请点击下方按钮添加"
                    setTextColor(Color.GRAY); textSize = 14f; setPadding(20, 40, 20, 40); gravity =
                    Gravity.CENTER
                })
            } else {
                notedTurns.forEach { turnData ->
                    val row = LinearLayout(themeContext).apply {
                        ...
                        Gravity.CENTER_VERTICAL; setPadding(0, 15, 0, 15)
                    }
                    val tvInfo = TextView(themeContext).apply {
                        text = "T${turnData.turnNumber}: ${turnData.remark}"
                        textSize = 15f; setTextColor(Color.BLACK); layoutParams =
                    ...
                    val btnDelete = TextView(themeContext).apply {
                        ...
                        setOnClickListener {
                            turnData.remark = ""; refreshList(); onDataChanged()
        ...
        DialogUtils.safeShowOverlayDialog(
            AlertDialog.Builder(themeContext).setTitle("备注管理").setView(rootLayout)
    ...
    private fun showEditDialog(
        ...
        val etTurn = EditText(context).apply {
            hint = "第几回合 (例如: 1)"; inputType = InputType.TYPE_CLASS_NUMBER
            if (targetData != null) {
                setText(targetData.turnNumber.toString()); isEnabled = false
            }
        }
        val etContent =
            EditText(context).apply { hint = "请输入备注内容"; setText(targetData?.remark ?: "") }
        ...
        DialogUtils.safeShowOverlayDialog(
            ...
            .setTitle(if (targetData == null) "新增备注" else "编辑备注").setView(layout)
            .setPositiveButton("保存") { _, _ ->
                val turnStr = etTurn.text.toString();
                val content = etContent.text.toString()
                if (turnStr.isEmpty()) {
                    Toast.makeText(context, "请输入回合数", Toast.LENGTH_SHORT)
                ...
                if (target != null) {
                    target.remark = content; onSave(); Toast.makeText(
                        context,
                        "备注已保存",
                        Toast.LENGTH_SHORT
                    ).show()
