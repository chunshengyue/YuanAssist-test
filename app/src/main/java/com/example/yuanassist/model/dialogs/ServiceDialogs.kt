// 檔案路徑：yuanassist/ui/dialogs/ServiceDialogs.kt
...
object ServiceDialogs {
    ...
    fun showTextImportDialog(context: Context, onImport: (String) -> Unit) {
        ...
        val editText = EditText(themeContext).apply {
            hint = "请在此处粘贴文本..."
            setTextColor(Color.BLACK); minLines = 5; gravity = Gravity.TOP or Gravity.START
            disableShowSoftInput()
        }
        val btnPaste = Button(themeContext).apply {
            text =
                "📋 点击粘贴剪贴板内容"; setTextColor(Color.WHITE); background.setTint(0xFF1976D2.toInt())
            setOnClickListener {
                editText.requestFocus()
                if (editText.onTextContextMenuItem(android.R.id.paste)) Toast.makeText(
                    context,
                    "粘贴成功",
                    Toast.LENGTH_SHORT
                ).show()
                else Toast.makeText(context, "粘贴失败或剪贴板为空", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        DialogUtils.safeShowOverlayDialog(
            AlertDialog.Builder(themeContext).setTitle("导入文字")
                .setView(container)
    ...
    fun showInsertTurnDialog(context: Context, onInsert: (Int) -> Unit) {
        ...
        val etTurn = EditText(themeContext).apply {
            hint = "在第几回合后新增？(例如: 12)"; inputType =
            InputType.TYPE_CLASS_NUMBER; disableShowSoftInput()
        }
        layout.addView(etTurn)

        DialogUtils.safeShowOverlayDialog(
            AlertDialog.Builder(themeContext).setTitle("新增回合")
                .setView(layout)
                .setPositiveButton("确定") { _, _ ->
                    val turnStr = etTurn.text.toString()
                    if (turnStr.isNotEmpty()) onInsert(turnStr.toInt()) else Toast.makeText(
                        context,
                        "请输入回合数",
                        Toast.LENGTH_SHORT
                    ).show()
    ...
    fun showEditActionDialog(
        ...
        val themeContext = DialogUtils.getThemeContext(context)
        val editText = EditText(themeContext).apply {
            setText(currentText); setTextColor(Color.BLACK); disableShowSoftInput(); setPadding(
            50,
            50,
            50,
            50
        ...
        DialogUtils.safeShowOverlayDialog(
            AlertDialog.Builder(themeContext).setTitle("编辑")
                .setView(editText)
    ...
    fun showExportImageSettingsDialog(context: Context, onExport: (Array<String>) -> Unit) {
        ...
        for (et in ets) {
            et.disableShowSoftInput(); et.isLongClickable =
                false; et.setOnLongClickListener { true }; et.setTextIsSelectable(false)
        }

        DialogUtils.safeShowOverlayDialog(
            AlertDialog.Builder(themeContext).setTitle("导出设置")
                .setView(dialogView)
                .setPositiveButton("生成图片") { _, _ -> onExport(Array(5) { i -> ets[i].text.toString() }) }
                .setNegativeButton("取消", null))
    }
    ...
    fun showSaveToLibraryDialog(context: Context, defaultName: String, onSave: (String) -> Unit) {
        ...
        val etName = EditText(themeContext).apply {
            hint = "请输入脚本名称"; setText(defaultName); disableShowSoftInput()
        }
        layout.addView(etName)

        DialogUtils.safeShowOverlayDialog(
            AlertDialog.Builder(themeContext).setTitle("导出到脚本库")
                .setView(layout)
                .setPositiveButton("确认") { _, _ ->
                    val scriptName = etName.text.toString().trim()
                    if (scriptName.isEmpty()) Toast.makeText(
                        context,
                        "名称不能为空",
                        Toast.LENGTH_SHORT
                    ).show() else onSave(scriptName)
                }.setNegativeButton("取消", null)
        )
    }
}